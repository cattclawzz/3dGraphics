...
def plotPoint(x, y, z, focalLength):
    return [focalLength*(x/z), focalLength*(y/z)]
...
def drawCube(x1,y1,z1, x2,y2,z2, focalLength):
    points = [[x1,y1], [x2,y1], [x2,y2], [x1,y2]]
    for i in range(4):
        drawLine(*points[i],z1, *points[(i+1)%4],z1, focalLength, 'red')
    for i in range(4):
        drawLine(*points[i],z2, *points[(i+1)%4],z2, focalLength, 'blue')

def render():
    canvas.delete("all")

    #drawSquare(100,100, 400,400)
    drawCube(100,100,100, 400,400,120, canZ)
